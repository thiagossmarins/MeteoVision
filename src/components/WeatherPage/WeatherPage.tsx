...
import { UVIndexCard } from "../UvIndexCard/UVIdexCard";
import { uvIndexToText } from "../../utils/uvText";
import { SolarDeclination } from "../SolarDeclination/SolarDeclination";
...
// as localizações salvas, sendo renderizado dentro de um PagerView.
...
export function WeatherPage({ weather, city, top, bottom }: WeatherPageProps) {
  ...
  const dailyForecast = useMemo(
    () => (weather ? getDailyForecast(weather.hourly) : []),
    [weather]
  );
  ...
  return (
    ...
    >
      ...
      <GlassBox mt="s16">
        ...
        <HourlyForecast weather={weather ?? undefined} />
      </GlassBox>

      <GlassBox mt="s16">
        <DailyForecast dailyForecast={dailyForecast} weather={weather ?? undefined} />
      </GlassBox>
      ...
      <Box mt="s16" flexDirection="row" alignItems="flex-start" justifyContent="space-between" gap="s16">
        <GlassBox flex={1} height={175}>
          ...
          <Text preset="smallFontSize" light>
            {humidityToText(weather?.hourly.relative_humidity_2m
              ? Math.max(...weather.hourly.relative_humidity_2m)
              : 0)}
          </Text>

          <Box alignItems="center" justifyContent="flex-end" height={"66%"}>
            <Text preset="mediumFontSize" textAlign="center" medium>
            ...
            <Box width={"100%"} height={16} backgroundColor="humidityBox" overflow="hidden" borderRadius="s8">
              <Box
                ...
                borderRadius={"s8"}
                style={{
                  width: `${weather?.hourly.relative_humidity_2m
                    ? Math.max(...weather.hourly.relative_humidity_2m)
                    : 0}%`,
                }}
              />
        ...
        <GlassBox flex={1} height={175}>
          ...
          <Text preset="smallFontSize" light>
            {weather ? uvIndexToText(Math.max(...weather.daily.uv_index_max)) : "0"}
          </Text>
